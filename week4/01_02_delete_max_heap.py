class MaxHeap:
    ...
    def delete(self):
        self.items[1], self.items[-1] = self.items[-1], self.items[1]  # 맨 위와 맨 아래 변경
    ...
    # 2. 맨 뒤에 있는 원소를 (원래 루트 노드)를 삭제합니다. 이 때, 끝에 반환해줘야 하니까 저장해둡시다.
    # 3. 변경된 노드와 자식 노드들을 비교합니다. 두 자식 중 더 큰 자식과 비교해서 자신보다 자식이 더 크다면 자리를 바꿔주자.
    # 4. 자식 노드 둘보다 부모 노드가 더 크거나 가장 바닥에 도달하지 않을 때까지 3.을 반복합니다.
